...
Python 侧的**外观级入侵屏蔽**（`DET_INTRUDER_MASK=1`，LAB 中值背景差分抓人手/白卡，黑纱 15/16→16/16）
**尚未**移植到 C#。若要移植需三段：OpenCvSharp 的 LAB 转换 + 逐像素中值背景；ΔE 阈值 + 开运算 + 连通域面积过滤；
映射到 patch 网格并膨胀。这部分是纯 CPU 整数/形态学运算，无 CPU/GPU 浮点分歧问题，一致性核验比 DINOv2 段容易。
决定移植的前提：在 F2 第二测试集上确认它不掉分。
