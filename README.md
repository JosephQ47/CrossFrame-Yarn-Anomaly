...
> **零训练、零缺陷样本、不使用任何标签。**
> 黑纱基准（71 帧 / 16 缺陷）：图级 AUROC **98.2** · AP **93.7** · 定位 **15/16** · PRO **76.8**；
> 用纯正常帧定阈时 召回 **93.8%** / 精确率 **83.3%**。
> 在此之前，改色增广、FDA、真实微调、PatchCore、GLASS 等 **10 余种方法在同一基准上全部 0/16**。
...
**数据集**：`D:/dataset/black_fabric`，71 帧 / 6 机位，其中 **16 帧含真缺陷（16 个实例）**、
55 帧正常。永久锁为测试集禁训。全部指标由 `python/metrics_full.py` 一次跑出，
原始结果 `data/metrics_黑纱基准.json`。
...
| **产线定阈**（纯正常帧 P95 = 12.3） | 召回 **93.8%** · 精确率 **83.3%** · F1 **88.2** | **这一行才是真实性能**：阈值只用正常帧定，完全没看缺陷帧 |
| TPR@FPR=5% | 93.8% | 误报压到 5% 时还能抓住多少 |
| FPR@TPR=100% | 7.3% | 一个都不漏时要承受的误报率 |
...
| 像素级 AUROC | 91.3 | **框掩膜近似**，仅筘齿带内统计（见下注） |
| **PRO** (FPR ≤ 0.3) | **76.8** | 每个缺陷区域等权，不被大缺陷主导 |
...
同一基准、同一口径（imgsz 1280, conf ≥ 0.25, detect IoU ≥ 0.3），
原始成绩 `data/benchmark_scores.csv`：

| 方法 | detect 召回 | 图级检出 |
|---|---|---|
| 纯白纱基线（YOLOv8n-seg） | 0.00 | 0/16 |
| FDA 频域域适应 | 0.00 | 0/16 |
| 合成改色（LCh） | 0.00 | 0/16 |
| 哑光改色 | 0.00 | 0/16 |
| 深色改色 | 0.00 | 0/16 |
| 真实黑纱微调 | 0.00 | 0/16 |
| PatchCore / GLASS 等通用异常检测 | — | 0/16（GLASS AUROC 58） |
| **跨帧同位置参考（本方法）** | — | **15/16** |

还有一组同样 0/16 的手工判据与自参考方案：structure-tensor 方向一致性、周期塌陷法、
同相位周期比对、**DINOv2 同图 k-NN**。
...
2. **参考池重污染**：24 帧里 15 帧含缺陷（黑纱基准是 71 帧里 16 帧，正常帧占 78%）；
3. 面积上限 55 是在黑纱人手块（57 patch）上调的，换机位换视场后这个先验不迁移。

**结论：跨帧参考的有效性取决于采集纪律，而不是纱线颜色。**
要在红纱上复现黑纱成绩，需要重采一批满足条件的数据——无人手入侵、无临时物件遮挡、
同机位连续多帧、正常帧占多数。

这个负结果被完整保留在仓库里，因为它是本方法适用条件的直接证据。
